...
%In this way, as seen in section \ref{}, with simple analytical models it is possible to analyze some basic feautures of the protein spectra. However, due to the high complexity of these systems, usually their internal dynamics is too complicated for a quantitative interpretation in terms of such models based almost only upon average information. 
...
\section{Molecular dynamics simulations of proteins}
...
\begin{minipage}[t]{0.725\textwidth}
	\centering
    \includegraphics[width=0.85\textwidth]{bounded_interactions.png}
...
%Structure databases contain the 3-D structures of biological macromolecules determined by X-ray crystallography and NMR. The primary structure database is the Protein Data Bank (PDB), an
...
\centering
\includegraphics[width=0.65\textwidth]{Prtoein_foldin_funnel-voet.PNG}
...
\begin{minipage}[t]{0.8\textwidth}
	\centering
    \includegraphics[width=0.85\textwidth]{initial_state-01.png}
...
\section{Simulations with NAMD and VMD}\label{sec:software}
...
In order to run any MD simulation, NAMD requires at least the following files \cite{ref:NAMD-VMD_Kukol}:
\begin{itemize}
...
\end{itemize}
During the simulation, several types of output data may be written by NAMD (see \ref{ssec:namd-ouput}). 
...
\subsubsection{Output files}\label{ssec:namd-ouput}
In output, for every simulation, NAMD returns two text files wrote following the PDB standards: 
...
the extensions for these files are (.coor) and (.vel), respectively. 
During the simulation, NAMD can also create a binary file that contains the atomic coordinates at several times, that are written over the course of a simulation. Thus, it provides a discrete trajectory of the system over the runtime. 

In addition, when NAMD runs, it also prints on the standard output a log of the simulation which it is often redirect into a (.log) text file. This file can hold important information on the progress of the simulation. In particular, the values of the energies (kinetic, bounded, unbounded and so forth), temperatures and pressures of the system during the simulation. The energy values are in kcal/mol. The pressure values are in bar. The temperature values are in Kelvin. 
