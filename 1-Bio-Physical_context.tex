...
In this picture, to figure how the function of a protein depends on its primary structure (i.e. its amino acid sequence), it is helpful to note that each protein has, at the same time: a unique three-dimensional structure that confers, at protein, a characteristic function and a specific sequence of amino acids that identifies it self uniquely.
...
Actually, the configurational entropy, along with the hydrogen-bonding interactions of many groups in the polypeptide chain with the solvent (water), tends to maintain the unfolded state. On the other hand, the chemical interactions that offset these effects and stabilize the native conformation include disulfide (covalent) bonds between and weak (noncovalent) interactions and forces, namely: hydrogen bonds, the hydrophobic effect, and ionic interactions. 

A disulfide bond, also called an S-S bond, or disulfide bridge, is a covalent bond derived, mainly, from two side-chains groups. 
