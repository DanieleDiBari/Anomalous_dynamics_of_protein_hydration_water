...
\centering
\includegraphics[width=0.65\textwidth]{Polymer-v2.png}

\caption{\small{Simple schematization of a section of a organic polymer such as DNA, RNA or proteins. $X_i$ represent the group of atoms of the monomers that build up the backbone. $R_i$ represent the side chain of the $i$-th monomers
...
Hence, this topic can be simplified by grouping the amino acids into distinct classes based on the properties of their R groups\footnote{Few amino acids are difficult to characterize and do not fit perfectly in any one group, particularly: glycine, histidine, and cysteine. Their assignments to particular groupings are the results of considered judgments rather than absolutes
...
The term ``residue'' reflects the loss of the elements of water when one amino acid is joined to another.

\textit{\textbf{POLYPEPTIDE STRUCTURE}}\\
Polypeptide chains make up proteins, which are one of the most important classes of biological macromolecules, having both structural and catalytic roles. Polypeptide chains are built up during
their biosynthesis from the 20 standard amino acids. These amino acids are linked together in a linear polypeptide chain by forming peptide bonds between them, in an order ordained by the nucleotide sequence of the corresponding gene for the protein. These 20 amino acid residues can also be modified post-translationally in very many different ways, to produce even
more variety. The amino acid sequence, appropriately known as the primary structure, identifies a protein unambiguously, determines all its chemical and biological properties, and specifies (indirectly) the higher levels of protein structure
\cite{creighton2010biophysical}.
...
\caption{\small{General scheme for formation of a dipeptide XY from two amino acids, X and Y, by reaction of the carboxyl group of amino acid X with the amino group of amino acid Y. \cite{creighton2010biophysical}.}}
